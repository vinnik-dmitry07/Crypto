...
from sub_tables import all_sub_tables, all_inv_tables, mds_vector, inverse_mds, \
    mix_col1, mix_col4, mix_col5, mix_col6, mix_col7, mix_col8, \
    mix_col173, mix_col149, mix_col118, mix_col168, mix_col47, mix_col73, mix_col215, mix_col202, rks_dec_128

all_mix_cols = [0, mix_col1, 0, 0, mix_col4, mix_col5, mix_col6, mix_col7, mix_col8]
all_inv_mix_cols = [mix_col173, mix_col149, mix_col118, mix_col168, mix_col47, mix_col73, mix_col215, mix_col202]
...
def to_bytes(input_list: Union[list, str], num_col: int = 8, c: int = 2, hex_key_length: int = 32, reverse: bool = False):
    """
    ...
    if type(parsed_int_key) == str:
        ...
        parsed_int_key = [int(pad_str[i:i + 2], 16) for i in range(0, len(pad_str), 2)]
    if reverse:
...
def LsbReturn(x: int, r: int, x_len: int = 64):
    ...
    if len(hex_str) < x_len:
        ...
        assert pad_str is not None, "Output is None. Either hex key length for padding is greater than key_length or " \
...
def add_method_simple(byte1: np.array, byte2: np.array, c: int = 2, hex_key_length: int = 32, modulus: int = 2**64):
    """
...
def add_method(k_obj: KalynaObject, byte2: np.array, modulus: int = 2**64):
    ...
    for i in range(length):
        ...
        f_block.append(v)
    f_block = ''.join(f_block)
    o_block = np.flipud(to_bytes(f_block, width, k_obj.NumStateMatrixRows, k_obj.HexKeyLength, True))
    k_obj.state = o_block
...
def xor_round_key(k_obj: KalynaObject, byte2: np.array):
    ...
    k_obj.state = to_bytes(xor_block, width, k_obj.NumStateMatrixRows, k_obj.HexKeyLength)
...
def sub_bytes(k_obj: KalynaObject, tables: list):
    ...
    sub_block = to_bytes([tables[j % 4][byte[j]] for byte in k_obj.state for j in range(len(byte))], num_col, c, h)
...
def rot_word(k_obj: KalynaObject, shift: list):
    ...
    """
    vals = k_obj.state
    assert len(shift) == vals.shape[1], "Length of shift list and state-matrix dim[1] must be same."
...
def mixColumns(k_obj: KalynaObject, tables: list, v: np.array):
    ...
    for byte in r_array:
        ...
        for row_v in range(len(byte)):
            ...
            g.append(pr)
    k_obj.state = to_bytes(g, k_obj.state.shape[1], k_obj.NumStateMatrixRows, k_obj.HexKeyLength)
...
def inv_sub_bytes(k_obj: KalynaObject, inv_tables: list):
    ...
    """
    num_col, c, h = k_obj.state.shape[1], k_obj.NumStateMatrixRows, k_obj.HexKeyLength
    inv_sub_block = to_bytes([inv_tables[j % 4][byte[j]] for byte in k_obj.state for j in range(len(byte))], num_col, c,
                             h)
...
def inv_rot_word(k_obj: KalynaObject, inv_shift: list):
    ...
    """
    vals = k_obj.state
    assert len(inv_shift) == vals.shape[1], "Length of inv_shift list and state-matrix dim[1] must be same."
    items = vals.transpose()
...
def KeyExpansionKT(k_obj: KalynaObject, main_key: int, k_alpha: np.array, k_omega: np.array, tables: list,
                   mix_col_t: list, c_mat: np.array, state_0: int):
    ...
    init_state_byte = to_bytes(init_state, k_obj.state.shape[1], k_obj.NumStateMatrixRows, k_obj.HexKeyLength, True)
...
def KeyExpansionEven(k_obj: KalynaObject, k_sigma: np.array, main_key: int, tables: list, mix_col_t: list,
                     c_mat: np.array):
    ...
    for i in range(0, k_obj.NumRounds + 1, 2):
        ...
        tmv = tmv_0 << (i // 2)
        tmv_byte = key_to_bytes(hex(tmv)[2:], k_obj.state.shape[1], k_obj.NumStateMatrixRows, k_obj.HexKeyLength)

        # right shift the main_key acc to following conditions
        if k == l:
            cipher_key = circular_right_shift(main_key, 32 * i, k)
            ckb = key_to_bytes(hex(cipher_key)[2:], k_obj.state.shape[1], k_obj.NumStateMatrixRows,
                               k_obj.PadKeyLength)
            ...
            if i % 4 == 0:
                ...
                ckb = key_to_bytes(hex(cipher_key)[2:], k_obj.state.shape[1], 2 * k_obj.NumStateMatrixRows,
                                   k_obj.PadKeyLength)
            ...
            cipher_key = main_key
            raise ValueError("Incorrect values of k:KeyLength = {} and l:BlockSize  = {} !".format(k, l))

        # Key expansion process
        kt_round = add_method_simple(k_sigma, tmv_byte, k_obj.NumStateMatrixRows, k_obj.HexKeyLength)
...
def EncryptBlock(k_obj: KalynaObject, p_text: Union[str, int], roundkeys: dict, tables: list, mix_col_t: list,
                c_mat: np.array):

    k_0 = roundkeys[0]
    k_obj.state = to_bytes(hex(p_text)[2:], k_obj.state.shape[1], k_obj.NumStateMatrixRows, k_obj.HexKeyLength, True)
    add_method(k_obj, k_0)

    for j in range(1, k_obj.NumRounds):
        sub_bytes(k_obj, tables)
        shift = [math.floor((i * k_obj.BlockSize) / 512) for i in range(len(k_obj.state[0]))]
        rot_word(k_obj, shift)
...
def DecryptBlock(k_obj: KalynaObject, c_text: Union[str, int], roundkeys: dict, inv_tables: list, inv_mix_col_t: list,
                inv_c_mat: np.array):

    l = k_obj.BlockSize
    t = k_obj.NumRounds
    k_t = roundkeys[t]
    k_obj.state = to_bytes(hex(c_text)[2:], k_obj.state.shape[1], k_obj.NumStateMatrixRows, k_obj.HexKeyLength, True)
...
def KalynaKeyExpansion(parameters: tuple, main_key: int, state_0: int):
    ...
    KT = 0
    if k == l:
        k0_b = key_to_bytes(hex(main_key)[2:], k_c.state.shape[1], c, k_c.PadKeyLength)  # k_alpha
        k1_b = key_to_bytes(hex(main_key)[2:], k_c.state.shape[1], c, k_c.PadKeyLength)  # k_omega
        KT = KeyExpansionKT(k_c, main_key, k0_b, k1_b, all_sub_tables, all_mix_cols, mds_matrix, state_0)
        k_c.state = KT
    elif k == 2 * l:
        # Number of State Matrix Rows i.e. k_obj.NumStateMatrixRows for key handling is different for k=2*l cases
        # k_alpha
    ...
    roundkeys = KeyExpansionEven(k_c, KT, main_key, all_sub_tables, all_mix_cols, mds_matrix)
    roundkeys = KeyExpansionOdd(k_c, roundkeys)

    return roundkeys


def KalynaEncrypt(parameters: tuple, p_text: Union[str, int], main_key: int, state_0: int):
...
def KalynaDecrypt(parameters: tuple, c_text: Union[str, int], main_key: int, state_0: int):

    l, k, t, c = parameters
    rks = KalynaKeyExpansion(parameters, main_key, state_0)
    k_d = KalynaObject(l, k, t, c)
    return DecryptBlock(k_d, c_text, rks, all_inv_tables, all_inv_mix_cols, inverse_mds_matrix)


# Make MDS matrix
v = np.array(list(map(int, mds_vector)))
mds_matrix = np.vstack([v, np.roll(v, 1), np.roll(v, 2), np.roll(v, 3), np.roll(v, 4),
                        np.roll(v, 5), np.roll(v, 6), np.roll(v, 7)])
v = np.array(list(map(int, inverse_mds)))

inverse_mds_matrix = np.vstack([v, np.roll(v, 1), np.roll(v, 2), np.roll(v, 3), np.roll(v, 4),
                                np.roll(v, 5), np.roll(v, 6), np.roll(v, 7)])
